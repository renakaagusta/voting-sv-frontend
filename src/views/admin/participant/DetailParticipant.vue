    ...
    <b-row>
      <b-col lg="6">
        <div class="container bg-white mt-2 p-3 shadow-sm rounded">
          ...
          <br />
          <b-button
            @click="send(detail.email, detail.name, detail.nim, detail._id)"
            class="ml-2"
            href=""
            variant="primary"
          >
          ...
          <b-button
            @click="
              download(
                'Kartu Pemilihan Pemira 2022_' + detail.name + '_' + detail.nim
              )
            "
            class="ml-2"
            href=""
          ...
          <router-link
            :to="{ name: 'EditParticipant', params: { id: detail._id } }"
          >
...
export default {
  ...
  methods: {
    ...
    send(to, name, nim, id) {
      ...
      htmlToImage
        .toPng(document.getElementById("pemira-card"))
        .then(function(image) {
          const data = {
            to: to,
            name: name,
